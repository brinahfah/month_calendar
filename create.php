...
<head>
    ...
    <style>
        ...
        form {
            ...
            max-width: 700px; /* Plus large pour les multiples entrées */
            margin: 20px auto;
        }
        .form-group {
            margin-bottom: 15px;
        ...
        .form-row {
            ...
            gap: 10px;
            margin-bottom: 10px;
            align-items: flex-end; /* Align les éléments en bas */
        }
        .form-row label {
            flex: 1; /* Permet aux labels de prendre de l'espace */
            min-width: 80px; /* Largeur minimale pour les labels */
        }
        .form-row input, .form-row select {
            flex: 2; /* Permet aux inputs/selects de prendre plus d'espace */
            padding: 8px;
            border: 1px solid #ddd;
            border-radius: 4px;
            box-sizing: border-box;
        }
        .form-row .input-wrapper { /* Wrapper pour les inputs */
            flex: 1;
        }
        .form-row .input-wrapper label {
            ...
            margin-bottom: 5px;
            font-weight: bold;
            flex: none; /* Ne pas laisser le label prendre tout l'espace */
        }
        .form-row .remove-btn {
                ...
                try {
                    ...
                    foreach ($jours_array as $index => $jour_saisi) {
                        ...
                        if (empty($jour) || empty($cours) || empty($heure)) {
                            $message_status .= "<p class='message error'>Ligne de programme incomplète à l'index " . ($index + 1) . ".</p>";
                            $all_success = false;
                            break; // Arrête le traitement si une ligne est invalide
                        }

                        // APPEL DE LA FONCTION POUR AJOUTER LE PROGRAMME (SANS id_week)
                        // Note: ajouterProgrammeSemaine prend maintenant un tableau d'id_schools
                        // Ici, nous passons un tableau avec un seul ID (celui sélectionné pour l'emploi du temps)
                    ...
                    if ($all_success) {
                        ...
                        $pdo->rollBack(); // Annule toutes les insertions en cas d'échec
                    ...
                    $pdo->rollBack(); // Annule la transaction en cas d'erreur PDO
