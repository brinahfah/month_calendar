 ...
 * @param array  $id_school Tableau des IDs des écoles/personnes à qui assigner le programme.
 * @return bool True si toutes les insertions réussissent, False sinon.
 */
function ajouterProgrammeSemaine(PDO $pdo, string $jour, string $cours, string $heure, array $id_school): bool
{
    try {
        ...
        foreach ($id_school as $id_ecole) {
            // Assurez-vous que l'ID de l'école est un entier valide
            $valid_id_ecole = filter_var($id_ecole, FILTER_VALIDATE_INT);
            if ($valid_id_ecole === false || $valid_id_ecole <= 0) {
                throw new PDOException("ID d'école invalide fourni pour l'assignation : " . $id_ecole);
            }
            $stmt_assign->bindParam(':id_week', $new_id_week, PDO::PARAM_INT);
            $stmt_assign->bindParam(':id_school', $valid_id_ecole, PDO::PARAM_INT);
            $stmt_assign->execute();
        }
